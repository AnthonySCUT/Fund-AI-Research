...
- 15 支被动 ETF 研究宇宙：沪深 300、中证 500、创业板、创业板 50、纳指、中证 1000、科创 50、证券、半导体、新能源、5G、医药、军工、光伏等；页面默认选中 12 支，可手动调整。
...
- 事件与证据页同时抓取东方财富普通公告流（`type=0`）和定期报告流（`type=3`），去重后分类展示年报、半年报、季报、招募说明书/基金合同、分红、治理、人事和关联交易等文档线索，并保留公告日期、原文链接与证据级别；公告索引不替代报告正文，AI 会明确要求打开原文核验。
- 五年十倍目标按数学门槛约 58.5% 年化收益进行压力测试；目标不是预测，也不是收益承诺。
...
- 指标对比表会额外展示历史年化收益距离五年十倍所需约 58.5% 年化门槛的差距；这是压力测试和筛选视图，不是收益预测。
- 结果顶部明确标注真实行情、真实接口回退或演示数据，并显示最新交易日和抓取完成时间；含演示数据时，候选组合会标记为“仅演示，不可执行”。
- 新增“基金详情”页：展示基金基本信息、最大回撤恢复时间、21/63/252 日滚动收益和研究样本收益相关性，辅助比较持有期稳定性和分散化效果。
